...
-- 1. 联动规则表
-- --------------------------------------------------------------
DROP TABLE IF EXISTS linkage_rules;
CREATE TABLE linkage_rules (
  ...
  trigger_condition JSON NOT NULL COMMENT '触发条件: {"alarmTypes":[1],"alarmLevels":[3,4],"deviceTypes":["烟感"],"locations":["1F"]}',
  linkage_action JSON NOT NULL COMMENT '联动动作: {"actions":[{"type":"video","target":"nearest","duration":30},{"type":"record","duration":60},{"type":"popup"},{"type":"notify","channels":["app","sms"]}]}',
  effective_time JSON DEFAULT NULL COMMENT '生效时间: {"type":"all"} 或 {"type":"custom","ranges":[{"start":"00:00","end":"23:59"}]}',
  priority INT DEFAULT 1 COMMENT '优先级: 1-10, 数字越小优先级越高',
...
CREATE TABLE linkage_records (
  ...
  action_results JSON DEFAULT NULL COMMENT '各动作执行结果: [{"type":"video","status":2,"msg":"成功调取摄像头","cameraId":"CAM001"}]',
...
-- 3. 初始化默认联动规则
-- --------------------------------------------------------------
INSERT INTO linkage_rules (rule_code, rule_name, rule_type, trigger_condition, linkage_action, effective_time, priority, status, description) VALUES
