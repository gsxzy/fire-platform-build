...
# 1. List all tables
_, out, _ = ssh.exec_command(f'{mysql_cmd} \"SHOW TABLES;\"')
tables_raw = out.read().decode('utf-8', errors='replace').strip().split('\n')
all_tables = [t.strip() for t in tables_raw[1:] if t.strip()]  # skip header
...
for tbl in expected_tables:
    if tbl in all_tables:
        _, out, _ = ssh.exec_command(f'{mysql_cmd} \"SELECT COUNT(*) FROM \\\"{tbl}\\\";\"')
        result = out.read().decode('utf-8', errors='replace').strip().split('\n')
        count = result[1].strip() if len(result) > 1 else 'N/A'
