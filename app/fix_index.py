...
# Move stubRouter before legacyRouter
old_block = """// 遗留兼容业务路由（数据迁移过渡）
...
new_block = """// 补充业务模块路由（兼容前端所有页面）—— 必须先挂载，避免被 legacyRouter 的通配符拦截
router.use(stubRouter);
...
if old_block in content:
    ...
    print('Fixed index.js order: stubRouter now mounted BEFORE legacyRouter')
else:
    print('Could not find expected block. Current content:')
    print(content)
