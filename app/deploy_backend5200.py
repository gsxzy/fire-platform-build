...
def deploy():
    ...
    try:
        ...
        print("Uploaded gb26875Server.js (port 5200)")

        cmds = [
            "iptables -D INPUT -p tcp --dport 9200 -j ACCEPT 2>/dev/null || true",
            "iptables -I INPUT -p tcp --dport 5200 -j ACCEPT || true",
            "iptables-save > /etc/iptables.rules 2>/dev/null || true",
        ]
        for cmd in cmds:
            stdin, stdout, stderr = client.exec_command(cmd)
            stdout.channel.recv_exit_status()
        print("Firewall updated: removed 9200, added 5200")
        ...
        if result:
            ...
            print("WARNING: Port 5200 NOT listening")

        stdin, stdout, stderr = client.exec_command("ss -tlnp | grep 9200")
        result = stdout.read().decode().strip()
        if not result:
            print("Port 9200 successfully stopped")
        else:
            print("Port 9200 still listening:")
            print(result)
