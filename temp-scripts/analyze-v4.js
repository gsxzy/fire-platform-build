...
function analyzeDeadCode(allFiles) {
  const exportsMap = {};
  const importsMap = {};
  const defaultImportsMap = {}; // importedFile -> [importingFiles] (files imported via default import)
  const reexportsMap = {};
    ...
    while ((m = importRegex.exec(content)) !== null) {
      ...
      if (m[1]) {
        ...
        for (const name of names) {
          ...
          importsMap[name].push({ file: relPath, line: lineNum, path: importPath });
        }
      }
      if (m[2]) {
        ...
        const lineNum = content.substring(0, m.index).split('\n').length;
        importsMap[alias].push({ file: relPath, line: lineNum, path: importPath });
      }
      if (m[3]) {
        const name = m[3].trim();
        if (!importsMap[name]) importsMap[name] = [];
        ...
        // Track default imports: import X from './file'
        if (!m[1] && !m[2]) {
          if (!defaultImportsMap[importPath]) defaultImportsMap[importPath] = [];
          defaultImportsMap[importPath].push({ file: relPath, line: lineNum, name });
        }
      }
    }
  }
  
  return { exportsMap, importsMap, reexportsMap, defaultImportsMap };
}

function resolveImportPath(importPath, fromFile) {
  // Very basic path resolution
  if (importPath.startsWith('@/')) {
    return importPath.slice(2) + '.ts';
  }
...
const { exportsMap, importsMap, reexportsMap, defaultImportsMap } = analyzeDeadCode(files);

// Build reexport chains
...
// Build default import chains: for each file, who imports it via default import
const defaultImportedBy = {};
for (const [importPath, importers] of Object.entries(defaultImportsMap)) {
  for (const imp of importers) {
    const resolved = resolveImportPath(importPath, imp.file);
    if (!defaultImportedBy[resolved]) defaultImportedBy[resolved] = [];
    defaultImportedBy[resolved].push(imp.file);
  }
}
...
for (const [name, exportInfos] of Object.entries(exportsMap)) {
  ...
  for (const info of exportInfos) {
    ...
    // Skip route files router variable (used as default export)
    if (info.file.includes('routes/') && info.name === 'router') continue;
    
    // Skip app.ts variable (entry point)
    if (info.file === 'app.ts' && info.name === 'app') continue;
    
    // Skip abstract keyword
    if (info.name === 'abstract') continue;
    
    // If it's a default export and the file is imported via default import, skip
    if (info.kind === 'default-export') {
      if (defaultImportedBy[info.file] && defaultImportedBy[info.file].length > 0) {
        continue;
      }
    }
    ...
    const exportFileName = info.file;
    const possibleSources = [exportFileName, './' + path.basename(exportFileName)];
    for (const source of possibleSources) {
      if (reexportChains[source]) {
        for (const reexporter of reexportChains[source]) {
          if (importsMap[name]) {
            for (const imp of importsMap[name]) {
              const resolvedPath = resolveImportPath(imp.path, imp.file);
              if (resolvedPath === reexporter || resolvedPath === path.dirname(reexporter) + '/index.ts') {
                isReexported = true;
                break;
              }
            }
          }
          if (isReexported) break;
        }
      }
      if (isReexported) break;
    }
...
fs.writeFileSync('D:\\新致远智慧消防平台\\fire-platform-build\\temp-scripts\\dead-code-report-v4.json', JSON.stringify(output, null, 2));
console.log('Done. Written to dead-code-report-v4.json');
