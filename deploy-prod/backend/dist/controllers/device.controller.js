...
exports.DeviceController = {
    async list(req, res) {
        try {
            ...
            if (needIot === 'true' || needIot === '1') {
                const [iotIds] = await database_1.default.query(`SELECT DISTINCT archive_device_id FROM fire_iot_device WHERE archive_device_id IS NOT NULL`, { type: 'SELECT' });
                const validIds = iotIds.map((r) => r.archive_device_id).filter(Boolean);
                if (validIds.length === 0) {
                    return res.json((0, response_1.page)([], 0, +pageNum, +pageSize));
                }
                where.id = { [sequelize_1.Op.in]: validIds };
            }
            ...
            if (deviceIds.length > 0) {
                const iotRows = await database_1.default.query(`SELECT archive_device_id, status, last_online FROM fire_iot_device WHERE archive_device_id IN (:ids)`, { replacements: { ids: deviceIds }, type: 'SELECT' });
    ...
    async create(req, res) {
        try {
            ...
            logger_1.default.error(`[Device] create 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
