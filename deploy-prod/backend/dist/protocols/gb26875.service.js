...
exports.gb26875Protocol = exports.GB26875ProtocolService = exports.ALARM_LEVELS = exports.SYSTEM_TYPES = exports.MSG_TYPES = void 0;
 ...
 * GB26875 协议解析服务
 */
class GB26875ProtocolService extends events_1.EventEmitter {
    static instance;
    constructor() {
        super();
    }
    ...
    parseFrame(frame) {
        ...
        if (frame.length < 7) {
            logger_1.default.debug(`[GB26875] 帧长度不足: ${frame.length} < 7`);
            return null;
        }
        // 验证起始符
        if (frame[0] !== 0x68) {
            logger_1.default.debug(`[GB26875] 无效起始符: 0x${frame[0].toString(16).toUpperCase()}`);
            return null;
        }
        ...
        if (length > 1000) {
            ...
            logger_1.default.debug(`[GB26875] 小端长度过大，切换大端解析: ${length}`);
        }
        // 验证总帧长
        const expectedTotalLen = length + 4; // 起始符(1) + 长度(2) + length + 结束符(1)
        if (frame.length < expectedTotalLen) {
            logger_1.default.debug(`[GB26875] 帧不完整: 期望${expectedTotalLen}字节，实际${frame.length}字节`);
            return null;
        }
        const addr = frame[3];
        const cmd = frame[4];
        ...
        // 验证结束符
        if (end !== 0x16) {
            logger_1.default.error(`[GB26875] 结束符错误: 0x${end.toString(16).toUpperCase()}, 期望 0x16`);
            return null;
        }
        // 验证校验和
        const calculatedChecksum = this.calcChecksum(frame.subarray(1, -2));
        if (calculatedChecksum !== checksum) {
            logger_1.default.error(`[GB26875] 校验错误: 计算=0x${calculatedChecksum.toString(16).toUpperCase()}, 接收=0x${checksum.toString(16).toUpperCase()}`);
            return null;
        }
        const cmdName = exports.MSG_TYPES[cmd] || `未知(0x${cmd.toString(16).toUpperCase()})`;
        return {
            length,
            addr,
            cmd,
     ...
     */
    async handleFrame(parsed, clientIp, socket) {
        logger_1.default.info(`\n[GB26875] [${clientIp}] 收到消息: ${parsed.cmdName}`);
        logger_1.default.info(`  长度: ${parsed.length}, 地址: ${parsed.addr}, 命令: 0x${parsed.cmd.toString(16).toUpperCase()}`);
        logger_1.default.debug(`  数据: ${parsed.data.toString('hex').toUpperCase()}`);
        // 根据命令类型处理
        switch (parsed.cmd) {
            case 0x01: // 注册上报
            ...
            case 0x07: // 手动报警
                ...
                break;
            default:
                logger_1.default.info(`  未处理的命令类型: ${parsed.cmdName}`);
        }
        // 发送通用应答
     ...
     * 处理注册上报
     */
    handleRegister(parsed, clientIp) {
        if (parsed.data.length >= 8) {
            ...
            logger_1.default.info(`  用户编码: ${userCode}`);
            logger_1.default.info(`  版本: ${version}`);
            logger_1.default.info(`  ✅ 注册成功 (用户编码: ${userCode})`);
            this.emit('event', {
                type: 'register',
                userCode,
                version,
     ...
     * 处理心跳包
     */
    handleHeartbeat(parsed, clientIp) {
        if (parsed.data.length >= 1) {
            ...
            logger_1.default.info(`  设备状态: 0x${status.toString(16).toUpperCase()}`);
            if (hasFire)
                logger_1.default.info(`    - ⚠️  有火警`);
            if (hasFault)
                logger_1.default.info(`    - ⚠️  有故障`);
            if (hasMainPowerFault)
                logger_1.default.info(`    - ⚠️  主电故障`);
            if (hasBackupPowerFault)
                logger_1.default.info(`    - ⚠️  备电故障`);
            logger_1.default.info(`  ✅ 心跳正常`);
            this.emit('event', {
                type: 'heartbeat',
                status,
                hasFire,
    ...
    handleEvent(parsed, clientIp) {
        ...
        logger_1.default.info(`  事件类型: ${alarmTypeName}`);
        if (parsed.data.length >= 4) {
            ...
            const systemTypeName = exports.SYSTEM_TYPES[systemType] || `未知(${systemType})`;
            logger_1.default.info(`  系统类型: ${systemTypeName}`);
            logger_1.default.info(`  位置: 回路${loop} 地址${device}`);
            logger_1.default.info(`  ⚠️  ${alarmTypeName}事件上报完成`);
            this.emit('event', {
                type: 'alarm',
    ...
    sendAck(socket, cmd) {
        try {
            ...
            socket.write(frame);
            logger_1.default.info(`[GB26875] 已发送通用应答 (cmd=0x${cmd.toString(16).toUpperCase()})`);
            return true;
        }
        catch (err) {
            logger_1.default.error(`[GB26875] 发送应答失败: ${err.message}`);
            return false;
        }
    }
    /**
    ...
    sendCheckPost(socket) {
        try {
            ...
            socket.write(frame);
            logger_1.default.info('[GB26875] 已发送查岗命令');
            return true;
        }
        catch (err) {
            logger_1.default.error(`[GB26875] 发送查岗失败: ${err.message}`);
            return false;
        }
    }
}
exports.GB26875ProtocolService = GB26875ProtocolService;
// 导出单例
exports.gb26875Protocol = GB26875ProtocolService.getInstance();
exports.default = exports.gb26875Protocol;
//# sourceMappingURL=gb26875.service.js.map