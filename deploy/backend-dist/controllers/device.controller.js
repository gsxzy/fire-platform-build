...
exports.DeviceController = {
    async list(req, res) {
        try {
            ...
            /* 数据一致性：仅返回有IoT接入配置的设备（设备配置页面使用） */
            const needIot = hasIotConfig ?? has_iot_config;
            let iotSubQuery = undefined;
            if (needIot === 'true' || needIot === '1') {
                iotSubQuery = database_1.default.where(database_1.default.literal(`(SELECT COUNT(*) FROM fire_iot_device iot WHERE iot.archive_device_id = fire_device.id)`), { [sequelize_1.Op.gt]: 0 });
            }
            const { count, rows } = await models_1.Device.findAndCountAll({
                where: iotSubQuery ? { [sequelize_1.Op.and]: [where, iotSubQuery] } : where,
                limit: +pageSize,
            ...
            return res.json((0, response_1.page)(list, count, +pageNum, +pageSize));
        }
        catch (err) {
            logger_1.default.error(`[Device] list 失败: ${err?.message || err}`);
    ...
    async create(req, res) {
        try {
            ...
            if (errors && errors.length > 0) {
                return res.status(400).json((0, response_1.fail)(errors.join('；'), 400));
            }
            if (payload.device_no) {
                const exist = await models_1.Device.findOne({ where: { device_no: payload.device_no } });
                if (exist) {
                    payload.device_no = generateDeviceNo(Math.floor(Math.random() * 900) + 100);
                }
            }
            const device = await models_1.Device.create(payload);
            logger_1.default.info(`[Device] 创建成功 id=${device.id} device_no=${payload.device_no}`);
            return res.json((0, response_1.success)({ id: device.id }, '创建成功'));
        }
        catch (err) {
            logger_1.default.error(`[Device] create 失败: ${err?.message || err}, body=${JSON.stringify(req.body)}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async update(req, res) {
        try {
            ...
            logger_1.default.error(`[Device] update 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async delete(req, res) {
        try {
            ...
            if (!Number.isFinite(id) || id <= 0) {
                return res.status(400).json((0, response_1.fail)('无效的设备ID', 400));
            }
            /* 先移除接入层，避免留下 archive_device_id 指向已删档案的孤儿或悬空引用 */
            await models_1.IoTDevice.destroy({ where: { archive_device_id: id } });
            await models_1.Device.destroy({ where: { id } });
            logger_1.default.info(`[Device] 删除成功 id=${id}`);
            return res.json((0, response_1.success)(null, '删除成功'));
        }
        catch (err) {
            logger_1.default.error(`[Device] delete 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async stats(req, res) {
        try {
            ...
            logger_1.default.error(`[Device] stats 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async types(req, res) {
        try {
            const types = await models_1.Device.findAll({
                attributes: ['device_type', [models_1.Device.sequelize.fn('COUNT', '*'), 'count']],
                group: ['device_type'], raw: true,
            });
            return res.json((0, response_1.success)(types));
        }
        catch (err) {
            logger_1.default.error(`[Device] types 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async scrap(req, res) {
        try {
            ...
            return res.json((0, response_1.success)(null, '报废成功'));
        }
        catch (err) {
            logger_1.default.error(`[Device] scrap 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async getConfig(req, res) {
        try {
            ...
            return res.json((0, response_1.success)(config));
        }
        catch (err) {
            logger_1.default.error(`[Device] getConfig 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
    async saveConfig(req, res) {
        try {
            ...
            return res.json((0, response_1.success)(null, '配置保存成功'));
        }
        catch (err) {
            logger_1.default.error(`[Device] saveConfig 失败: ${err?.message || err}`);
            return res.status(500).json((0, response_1.fail)(`操作失败: ${err?.message || '未知错误'}`, 500));
        }
    },
};
