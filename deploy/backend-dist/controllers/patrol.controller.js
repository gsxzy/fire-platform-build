...
exports.PatrolController = {
    ...
    async planCreate(req, res) {
        try {
            const p = await models_1.PatrolPlan.create(sanitizeBody(req.body));
    ...
    async planUpdate(req, res) {
        try {
            await models_1.PatrolPlan.update(sanitizeBody(req.body), { where: { id: sanitizeId(req.params.id) } });
    ...
    async planDelete(req, res) {
        try {
            await models_1.PatrolPlan.destroy({ where: { id: sanitizeId(req.params.id) } });
    ...
    async recordCreate(req, res) {
        try {
            ...
            const r = await models_1.PatrolRecord.create({ ...sanitizeBody(req.body), patrol_no: patrolNo });
    ...
    async recordUpdate(req, res) {
        try {
            await models_1.PatrolRecord.update(sanitizeBody(req.body), { where: { id: sanitizeId(req.params.id) } });
            return res.json((0, response_1.success)(null, '更新成功'));
        }
        catch (err) {
    ...
    async recordDelete(req, res) {
        try {
            await models_1.PatrolRecord.destroy({ where: { id: sanitizeId(req.params.id) } });
            return res.json((0, response_1.success)(null, '删除成功'));
        }
        catch (err) {
    ...
    async hazardCreate(req, res) {
        try {
            ...
            const h = await models_1.Hazard.create({ ...sanitizeBody(req.body), hazard_no: hazardNo });
    ...
    async hazardUpdate(req, res) {
        try {
            await models_1.Hazard.update(sanitizeBody(req.body), { where: { id: sanitizeId(req.params.id) } });
            return res.json((0, response_1.success)(null, '更新成功'));
        }
        catch (err) {
    ...
    async hazardDelete(req, res) {
        try {
            await models_1.Hazard.destroy({ where: { id: sanitizeId(req.params.id) } });
            return res.json((0, response_1.success)(null, '删除成功'));
        }
        catch (err) {
    ...
    async hazardRectify(req, res) {
        try {
            await models_1.Hazard.update({ status: 2, rectification_date: new Date() }, { where: { id: sanitizeId(req.params.id) } });
