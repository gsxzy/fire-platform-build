...
print(f'Total ctwing_raw_log: {total}')

cur.execute("SELECT device_id FROM ctwing_raw_log GROUP BY device_id")
devices = [r['device_id'] for r in cur.fetchall()]

migrated = 0
for dev in devices:
    cur.execute("SELECT created_at, msg_type, raw_json FROM ctwing_raw_log WHERE device_id=%s ORDER BY created_at", (dev,))
    rows = cur.fetchall()
    safe_dev = str(dev).replace("'", "\\'").replace(' ', '_')[:50]
    subtable = f"ctb_raw_ctwing_{safe_dev}"

    vals = []
    for row in rows:
        ts = row['created_at'].strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] if row['created_at'] else ''
        cmd_type = str(row.get('msg_type') or '').replace("'", "\\'")[:16]
        raw_json = str(row.get('raw_json') or '').replace("'", "\\'")[:4000]
        vals.append(f"('{ts}', '', '{cmd_type}', '', '{raw_json}')")

    for i in range(0, len(vals), 500):
        chunk = vals[i:i+500]
        sql = f"INSERT INTO {subtable} USING stb_raw_log TAGS ('ctwing', '{safe_dev}') VALUES {', '.join(chunk)};"
        try:
            td_exec(sql)
            migrated += len(chunk)
        except Exception as e:
            print(f"  ERROR {safe_dev}: {e}")
