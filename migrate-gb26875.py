...
cur.execute("SELECT COUNT(*) as cnt FROM gb26875_raw_log")
total = cur.fetchone()['cnt']
print(f'Total gb26875_raw_log: {total}')

cur.execute("SELECT device_id FROM gb26875_raw_log GROUP BY device_id")
...
dir_map = {'1': 'RX', '2': 'TX'}
for dev in devices:
    cur.execute("SELECT created_at, direction, cmd_type, hex_data, parsed_json FROM gb26875_raw_log WHERE device_id=%s ORDER BY created_at", (dev,))
    rows = cur.fetchall()
    safe_dev = str(dev).replace("'", "\\'").replace(' ', '_')[:50]
    subtable = f"ctb_raw_gb26875_{safe_dev}"

    vals = []
    for row in rows:
        ts = row['created_at'].strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] if row['created_at'] else ''
        direction = dir_map.get(str(row.get('direction')), str(row.get('direction') or '')).replace("'", "\\'")[:8]
    ...
    for i in range(0, len(vals), 500):
        chunk = vals[i:i+500]
        sql = f"INSERT INTO {subtable} USING stb_raw_log TAGS ('gb26875', '{safe_dev}') VALUES {', '.join(chunk)};"
        try:
            td_exec(sql)
            migrated += len(chunk)
            ...
            for v in chunk:
                try:
                    td_exec(f"INSERT INTO {subtable} USING stb_raw_log TAGS ('gb26875', '{safe_dev}') VALUES {v};")
                    migrated += 1
                except:
                    pass
