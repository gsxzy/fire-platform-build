...
for (const file of files) {
  ...
  // 1. BIGINT.UNSIGNED -> BIGINT (pure ASCII, safe)
  content = content.replace(/DataTypes\.BIGINT\.UNSIGNED/g, 'DataTypes.BIGINT');

  // 2. TEXT('long') -> TEXT (pure ASCII, safe)
  content = content.replace(/DataTypes\.TEXT\('long'\)/g, 'DataTypes.TEXT');

  // 3. JSON -> JSONB ONLY in DataTypes.JSON context (avoid JSON.parse etc.)
  content = content.replace(/DataTypes\.JSON(?!B)/g, 'DataTypes.JSONB');

  // 4. Table name fixes - only match exact tableName assignments
