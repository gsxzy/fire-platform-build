/**
 * ═══════════════════════════════════════════════════════════════════
 * 数据迁移脚本：旧版 JS 后端表 → 新版 TS 后端表
 * 运行方式：npx tsx scripts/migration.ts
 * 注意：生产环境运行前请务必备份数据库！
 * ═══════════════════════════════════════════════════════════════════
 */
...
/* ───── 工具函数 ───── */
function parseIntSafe(v: any, def = 0): number {
  const n = parseInt(v);
  return isNaN(n) ? def : n;
}

function parseFloatSafe(v: any, def = 0): number {
...
function mapStatus(oldStatus: string | number | null, mapping: Record<string, number>): number {
  if (oldStatus === null || oldStatus === undefined) return 1;
...
function hashPassword(pwd: string | null | undefined): string {
  if (!pwd) return bcrypt.hashSync('default123', 10);
  // 如果已经是 bcrypt 格式，直接保留
  if (/^\$2[aby]\$\d+\$/.test(pwd)) return pwd;
  return bcrypt.hashSync(pwd, 10);
}

let alarmCounter = 0;
function generateAlarmNo(row: any): string {
  if (row.alarm_no) return row.alarm_no;
  return `ALM${Date.now()}_${alarmCounter++}_${Math.random().toString(36).slice(2, 8)}`;
}

let woCounter = 0;
function generateWorkOrderNo(row: any): string {
...
async function initMappingTable() {
  ...
  `);
  logger.info('[Migration] ID 映射临时表已就绪');
}

async function saveMap(tableName: string, oldId: string, newId: number) {
  await sequelize.query(
...
async function getNewId(tableName: string, oldId: string): Promise<number | null> {
  const [rows] = await sequelize.query(
    `SELECT new_id FROM _migration_id_map WHERE table_name = ? AND old_id = ?`,
    { replacements: [tableName, oldId] }
  );
  return (rows as any[])[0]?.new_id || null;
}

async function clearMapping() {
  await sequelize.query(`TRUNCATE TABLE _migration_id_map`);
}

/* ───── 迁移前：自动检测并重命名旧版同名表 ───── */
async function prepareLegacyTables() {
  logger.info('[Migration] 检查并准备旧版同名表...');

  // fire_device：旧版是主机回路点位表，没有 device_no 列
  const [deviceTables] = await sequelize.query(`SHOW TABLES LIKE 'fire_device'`);
  if ((deviceTables as any[]).length > 0) {
    const [cols] = await sequelize.query(`SHOW COLUMNS FROM fire_device`);
    const hasDeviceNo = (cols as any[]).some((c: any) => c.Field === 'device_no');
    if (!hasDeviceNo) {
      await sequelize.query(`RENAME TABLE fire_device TO fire_device_legacy`);
      logger.info('[Migration] 旧 fire_device 已重命名为 fire_device_legacy');
    } else {
      logger.info('[Migration] fire_device 已是新版 schema，跳过重命名');
    }
  }

  // fire_alarm：旧版没有 alarm_desc 列
  const [alarmTables] = await sequelize.query(`SHOW TABLES LIKE 'fire_alarm'`);
  if ((alarmTables as any[]).length > 0) {
    const [cols] = await sequelize.query(`SHOW COLUMNS FROM fire_alarm`);
    const hasAlarmDesc = (cols as any[]).some((c: any) => c.Field === 'alarm_desc');
    if (!hasAlarmDesc) {
      await sequelize.query(`RENAME TABLE fire_alarm TO fire_alarm_legacy`);
      logger.info('[Migration] 旧 fire_alarm 已重命名为 fire_alarm_legacy');
    } else {
      logger.info('[Migration] fire_alarm 已是新版 schema，跳过重命名');
    }
  }
}
...
async function migrateDepartments() {
  logger.info('[Migration] 开始迁移 departments → sys_department');
  const [oldRows] = await sequelize.query('SELECT * FROM departments');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      ...
      logger.error(`[Migration] departments 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] departments → sys_department 完成: ${count} 条`);
}
...
async function migrateUnits() {
  logger.info('[Migration] 开始迁移 units → fire_unit');
  const [oldRows] = await sequelize.query('SELECT * FROM units');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitTypeMap: Record<string, number> = { 'key': 1, 'general': 2, 'nine-small': 3, '重点单位': 1, '一般单位': 2, '九小场所': 3 };
      const fireLevelMap: Record<string, number> = { '一级': 1, '二级': 2, '三级': 3, '1': 1, '2': 2, '3': 3 };
      const [result] = await sequelize.query(
      ...
      logger.error(`[Migration] units 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] units → fire_unit 完成: ${count} 条`);
}
...
async function migrateUsers() {
  logger.info('[Migration] 开始迁移 users → sys_user');
  const [oldRows] = await sequelize.query('SELECT * FROM users');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      ...
      logger.error(`[Migration] users 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] users → sys_user 完成: ${count} 条`);
}
...
async function migrateDevices() {
  logger.info('[Migration] 开始迁移 device_archive → fire_device');
  const [oldRows] = await sequelize.query('SELECT * FROM device_archive');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = await getNewId('fire_unit', String(row.unit_id));
      const statusMap: Record<string, number> = { normal: 1, fault: 2, offline: 3, scrapped: 4, '1': 1, '2': 2, '3': 3 };
      const [result] = await sequelize.query(
      ...
      logger.error(`[Migration] device_archive 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] device_archive → fire_device 完成: ${count} 条`);
}
...
async function migrateAlarms() {
  logger.info('[Migration] 开始迁移 fire_alarm(旧) → fire_alarm(新)');

  const [legacyTables] = await sequelize.query(`SHOW TABLES LIKE 'fire_alarm_legacy'`);
  if ((legacyTables as any[]).length === 0) {
    logger.info('[Migration] fire_alarm_legacy 不存在，跳过告警迁移');
    return;
  }

  const [oldRows] = await sequelize.query('SELECT * FROM fire_alarm_legacy');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      ...
      const unitNewId = row.unit_id && row.unit_id !== 'PENDING'
        ...
        : null;
      const alarmTypeMap: Record<string, number> = {
        '火警': 1, 'fire': 1, '1': 1,
        '故障': 2, 'fault': 2, '2': 2,
        '预警': 3, 'pre': 3, '3': 3,
        '屏蔽': 4, 'shield': 4, '4': 4,
      ...
      logger.error(`[Migration] fire_alarm 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] fire_alarm(旧) → fire_alarm(新) 完成: ${count} 条`);
}
...
async function migrateWorkOrders() {
  logger.info('[Migration] 开始迁移 work_orders → fire_maint_work_order');
  const [oldRows] = await sequelize.query('SELECT * FROM work_orders');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      const statusMap: Record<string, number> = { pending: 0, processing: 1, done: 2, closed: 3, '0': 0, '1': 1, '2': 2, '3': 3 };
      const orderTypeMap: Record<string, number> = { repair: 1, maintain: 2, inspect: 3, '1': 1, '2': 2, '3': 3 };
      const [result] = await sequelize.query(
      ...
      logger.error(`[Migration] work_orders 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] work_orders → fire_maint_work_order 完成: ${count} 条`);
}
...
async function migratePatrolPlans() {
  logger.info('[Migration] 开始迁移 patrol_plans → fire_patrol_plan');
  const [oldRows] = await sequelize.query('SELECT * FROM patrol_plans');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      const [result] = await sequelize.query(
        `INSERT INTO fire_patrol_plan
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.plan_name || row.name || '未命名', unitNewId,
      ...
      logger.error(`[Migration] patrol_plans 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] patrol_plans → fire_patrol_plan 完成: ${count} 条`);
}
...
async function migratePatrolRecords() {
  logger.info('[Migration] 开始迁移 patrol_records → fire_patrol_record');
  const [oldRows] = await sequelize.query('SELECT * FROM patrol_records');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      ...
      logger.error(`[Migration] patrol_records 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] patrol_records → fire_patrol_record 完成: ${count} 条`);
}
...
async function migrateHazards() {
  logger.info('[Migration] 开始迁移 hazards → fire_hazard');
  const [oldRows] = await sequelize.query('SELECT * FROM hazards');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      ...
      logger.error(`[Migration] hazards 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] hazards → fire_hazard 完成: ${count} 条`);
}
...
async function migratePlans() {
  logger.info('[Migration] 开始迁移 plans → fire_emergency_plan');
  const [oldRows] = await sequelize.query('SELECT * FROM plans');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      const [result] = await sequelize.query(
        `INSERT INTO fire_emergency_plan
         ...
         VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.name || '未命名', unitNewId, parseIntSafe(row.plan_type) || 1,
          row.content || null,
          mapStatus(row.status, { active: 1, inactive: 0, '1': 1, '0': 0 }),
        ]}
      );
      ...
      logger.error(`[Migration] plans 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] plans → fire_emergency_plan 完成: ${count} 条`);
}
...
async function migrateDrills() {
  logger.info('[Migration] 开始迁移 drills → fire_emergency_drill');
  const [oldRows] = await sequelize.query('SELECT * FROM drills');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      ...
      logger.error(`[Migration] drills 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] drills → fire_emergency_drill 完成: ${count} 条`);
}
...
async function migrateInspections() {
  logger.info('[Migration] 开始迁移 inspections → fire_inspection');
  const [oldRows] = await sequelize.query('SELECT * FROM inspections');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const unitNewId = row.unit_id ? await getNewId('fire_unit', String(row.unit_id)) : null;
      ...
      logger.error(`[Migration] inspections 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] inspections → fire_inspection 完成: ${count} 条`);
}
...
async function migrateDocuments() {
  logger.info('[Migration] 开始迁移 documents → fire_knowledge_doc');
  const [oldRows] = await sequelize.query('SELECT * FROM documents');
  let count = 0;
  for (const row of oldRows as any[]) {
    try {
      const [result] = await sequelize.query(
        `INSERT INTO fire_knowledge_doc
         ...
         VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.title || row.name || '未命名', parseIntSafe(row.doc_type) || 1,
      ...
      logger.error(`[Migration] documents 迁移失败 id=${row.id}: ${err.message}`);
    }
  }
  logger.info(`[Migration] documents → fire_knowledge_doc 完成: ${count} 条`);
}
...
async function runMigration() {
  logger.info('═══════════════════════════════════════════════════════');
  logger.info('  数据迁移开始：旧版表 → 新版表');
  logger.info('  ⚠️  生产环境运行前请务必备份数据库！');
  logger.info('═══════════════════════════════════════════════════════');

  try {
    await sequelize.authenticate();
    logger.info('[Migration] 数据库连接成功');
    ...
    // 按依赖顺序迁移
    await migrateDepartments();   // 无依赖
    await migrateUnits();         // 无依赖
    await migrateUsers();         // 依赖 department（简化处理为 0）
    await migrateDevices();       // 依赖 unit
    ...
    await migrateDocuments();     // 无依赖

    logger.info('═══════════════════════════════════════════════════════');
    logger.info('  ✅ 数据迁移全部完成');
    logger.info('═══════════════════════════════════════════════════════');
  } catch (err: any) {
    logger.error(`[Migration] 迁移失败: ${err.message}`);
    throw err;
  } finally {
