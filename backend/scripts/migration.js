        ...
        for (let x = this.maxFiles - 1; x > 1; x--) {
          tasks.push(function(i, cb) {
            ...
            fs.exists(tmppath, (exists) => {
              ...
              fs.rename(tmppath, path2.join(this.dirname, fileName), cb);
            });
          }.bind(this, x));
        }
        asyncSeries(tasks, () => {
  ...
  "node_modules/winston-daily-rotate-file/daily-rotate-file.js"(exports2, module2) {
    ...
    var DailyRotateFile2 = function(options) {
      ...
      if (options.stream) {
        ...
        this.logStream = require_FileStreamRotator().getStream({
          ...
          end_stream: true,
          audit_file: options.auditFile ? options.auditFile : path2.join(this.dirname, "." + hash(options) + "-audit.json"),
    ...
    DailyRotateFile2.prototype.query = function(options, callback) {
      ...
      const logFiles = (() => {
        ...
        return fs.readdirSync(this.dirname).filter((file) => path2.basename(file).match(fileRegex));
      })();
...
var sequelize = new import_sequelize.Sequelize(
  ...
  {
    ...socketPath ? { dialectOptions: { socketPath } } : { host, port },
    dialect: "mysql",
    pool: {
...
var logger = import_winston.default.createLogger({
  ...
  transports: [
    new import_winston_daily_rotate_file.default({
      filename: import_path.default.join(LOG_DIR, "error-%DATE%.log"),
    ...
    new import_winston_daily_rotate_file.default({
      filename: import_path.default.join(LOG_DIR, "combined-%DATE%.log"),
      datePattern: "YYYY-MM-DD",
      maxSize: "50m",
      maxFiles: "30d"
    })
...
function mapStatus(oldStatus, mapping) {
  if (oldStatus === null || oldStatus === void 0) return 1;
  const s = String(oldStatus).toLowerCase().trim();
  if (mapping[s] !== void 0) return mapping[s];
  const n = parseIntSafe(oldStatus);
  return isNaN(n) ? 1 : n;
}
function hashPassword(pwd) {
  if (!pwd) return import_bcryptjs.default.hashSync("default123", 10);
  if (/^\$2[aby]\$\d+\$/.test(pwd)) return pwd;
  return import_bcryptjs.default.hashSync(pwd, 10);
}
var alarmCounter = 0;
function generateAlarmNo(row) {
  if (row.alarm_no) return row.alarm_no;
...
async function initMappingTable() {
  ...
  `);
  logger_default.info("[Migration] ID \u6620\u5C04\u4E34\u65F6\u8868\u5DF2\u5C31\u7EEA");
}
async function saveMap(tableName, oldId, newId) {
  await database_default.query(
    `INSERT INTO _migration_id_map (table_name, old_id, new_id) VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE new_id = VALUES(new_id)`,
    { replacements: [tableName, oldId, newId] }
  );
}
async function getNewId(tableName, oldId) {
  const [rows] = await database_default.query(
    `SELECT new_id FROM _migration_id_map WHERE table_name = ? AND old_id = ?`,
    { replacements: [tableName, oldId] }
  );
  return rows[0]?.new_id || null;
}
async function clearMapping() {
  await database_default.query(`TRUNCATE TABLE _migration_id_map`);
}
async function prepareLegacyTables() {
  logger_default.info("[Migration] \u68C0\u67E5\u5E76\u51C6\u5907\u65E7\u7248\u540C\u540D\u8868...");
  const [deviceTables] = await database_default.query(`SHOW TABLES LIKE 'fire_device'`);
  if (deviceTables.length > 0) {
    const [cols] = await database_default.query(`SHOW COLUMNS FROM fire_device`);
    const hasDeviceNo = cols.some((c) => c.Field === "device_no");
    if (!hasDeviceNo) {
      await database_default.query(`RENAME TABLE fire_device TO fire_device_legacy`);
      logger_default.info("[Migration] \u65E7 fire_device \u5DF2\u91CD\u547D\u540D\u4E3A fire_device_legacy");
    } else {
      logger_default.info("[Migration] fire_device \u5DF2\u662F\u65B0\u7248 schema\uFF0C\u8DF3\u8FC7\u91CD\u547D\u540D");
    }
  }
  const [alarmTables] = await database_default.query(`SHOW TABLES LIKE 'fire_alarm'`);
  if (alarmTables.length > 0) {
    const [cols] = await database_default.query(`SHOW COLUMNS FROM fire_alarm`);
    const hasAlarmDesc = cols.some((c) => c.Field === "alarm_desc");
    if (!hasAlarmDesc) {
      await database_default.query(`RENAME TABLE fire_alarm TO fire_alarm_legacy`);
      logger_default.info("[Migration] \u65E7 fire_alarm \u5DF2\u91CD\u547D\u540D\u4E3A fire_alarm_legacy");
    } else {
      logger_default.info("[Migration] fire_alarm \u5DF2\u662F\u65B0\u7248 schema\uFF0C\u8DF3\u8FC7\u91CD\u547D\u540D");
    }
  }
}
async function migrateDepartments() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB departments \u2192 sys_department");
  const [oldRows] = await database_default.query("SELECT * FROM departments");
  let count = 0;
  for (const row of oldRows) {
    try {
      const [result] = await database_default.query(
        `INSERT INTO sys_department (dept_name, parent_id, leader, phone, sort, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.name,
          parseIntSafe(row.parent_id) || 0,
          row.leader || null,
          row.phone || null,
          parseIntSafe(row.sort_order) || 0,
      ...
      logger_default.error(`[Migration] departments \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] departments \u2192 sys_department \u5B8C\u6210: ${count} \u6761`);
}
async function migrateUnits() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB units \u2192 fire_unit");
  const [oldRows] = await database_default.query("SELECT * FROM units");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitTypeMap = { "key": 1, "general": 2, "nine-small": 3, "\u91CD\u70B9\u5355\u4F4D": 1, "\u4E00\u822C\u5355\u4F4D": 2, "\u4E5D\u5C0F\u573A\u6240": 3 };
      const fireLevelMap = { "\u4E00\u7EA7": 1, "\u4E8C\u7EA7": 2, "\u4E09\u7EA7": 3, "1": 1, "2": 2, "3": 3 };
      const [result] = await database_default.query(
        ...
        { replacements: [
          row.name,
          row.id,
          unitTypeMap[row.type] || 2,
          row.address || null,
          parseFloatSafe(row.lng),
          parseFloatSafe(row.lat),
          row.contact_name || null,
          row.contact_phone || null,
          parseFloatSafe(row.area),
          parseIntSafe(row.total_floors),
      ...
      logger_default.error(`[Migration] units \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] units \u2192 fire_unit \u5B8C\u6210: ${count} \u6761`);
}
async function migrateUsers() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB users \u2192 sys_user");
  const [oldRows] = await database_default.query("SELECT * FROM users");
  let count = 0;
  for (const row of oldRows) {
    try {
      ...
      logger_default.error(`[Migration] users \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] users \u2192 sys_user \u5B8C\u6210: ${count} \u6761`);
}
async function migrateDevices() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB device_archive \u2192 fire_device");
  const [oldRows] = await database_default.query("SELECT * FROM device_archive");
  let count = 0;
  for (const row of oldRows) {
    try {
      ...
      const [result] = await database_default.query(
        ...
        { replacements: [
          row.code || row.id,
          row.name,
          row.category || row.sub_category || "unknown",
          row.model || null,
          ...
          parseFloatSafe(row.lng),
          parseFloatSafe(row.lat),
          toDate(row.install_date),
          toDate(row.expire_date),
          statusMap[String(row.status)] || 1,
          null,
          row.protocol_device_id || null,
          row.protocol_type || null
        ] }
      );
      const newId = result.insertId;
      ...
      logger_default.error(`[Migration] device_archive \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] device_archive \u2192 fire_device \u5B8C\u6210: ${count} \u6761`);
}
async function migrateAlarms() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB fire_alarm(\u65E7) \u2192 fire_alarm(\u65B0)");
  const [legacyTables] = await database_default.query(`SHOW TABLES LIKE 'fire_alarm_legacy'`);
  if (legacyTables.length === 0) {
    logger_default.info("[Migration] fire_alarm_legacy \u4E0D\u5B58\u5728\uFF0C\u8DF3\u8FC7\u544A\u8B66\u8FC1\u79FB");
    return;
  }
  const [oldRows] = await database_default.query("SELECT * FROM fire_alarm_legacy");
  let count = 0;
  for (const row of oldRows) {
    try {
      const deviceNewId = row.device_id ? await getNewId("fire_device", String(row.device_id)) : null;
      const unitNewId = row.unit_id && row.unit_id !== "PENDING" ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const alarmTypeMap = {
        "\u706B\u8B66": 1,
        "fire": 1,
        "1": 1,
      ...
      const [result] = await database_default.query(
        ...
        { replacements: [
          alarmNo,
          alarmTypeMap[String(row.alarm_type)] || 5,
          parseIntSafe(row.alarm_level) || 1,
          deviceNewId,
          row.device_name || null,
          unitNewId,
          row.unit_name && row.unit_name !== "\u5F85\u5206\u914D\u5355\u4F4D" ? row.unit_name : null,
          row.location || null,
          row.description || row.location || null,
          statusMap[String(row.alarm_status)] || 0,
          row.handler_id ? parseIntSafe(row.handler_id) : null,
          row.handler_name || null,
          toDate(row.confirm_time),
          row.handle_remark || null,
          toDate(row.confirm_time),
          0
        ] }
      );
      ...
      logger_default.error(`[Migration] fire_alarm \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] fire_alarm(\u65E7) \u2192 fire_alarm(\u65B0) \u5B8C\u6210: ${count} \u6761`);
}
async function migrateWorkOrders() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB work_orders \u2192 fire_maint_work_order");
  const [oldRows] = await database_default.query("SELECT * FROM work_orders");
  let count = 0;
  for (const row of oldRows) {
    try {
      ...
      const [result] = await database_default.query(
        `INSERT INTO fire_maint_work_order
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          generateWorkOrderNo(row),
          orderTypeMap[String(row.order_type)] || 1,
          null,
          unitNewId,
          row.fault_desc || null,
          parseIntSafe(row.priority) || 1,
          statusMap[String(row.status)] || 0,
          null,
          toDate(row.plan_start),
          toDate(row.plan_end),
          toDate(row.actual_start),
          toDate(row.actual_end),
          parseFloatSafe(row.material_cost),
          parseFloatSafe(row.labor_cost),
      ...
      logger_default.error(`[Migration] work_orders \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] work_orders \u2192 fire_maint_work_order \u5B8C\u6210: ${count} \u6761`);
}
async function migratePatrolPlans() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB patrol_plans \u2192 fire_patrol_plan");
  const [oldRows] = await database_default.query("SELECT * FROM patrol_plans");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const [result] = await database_default.query(
        `INSERT INTO fire_patrol_plan
         (plan_name, unit_id, patrol_type, patrol_items, responsible_id,
          start_date, end_date, cron_expr, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.plan_name || row.name || "\u672A\u547D\u540D",
          unitNewId,
          parseIntSafe(row.patrol_type) || 1,
          row.patrol_items || null,
      ...
      logger_default.error(`[Migration] patrol_plans \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] patrol_plans \u2192 fire_patrol_plan \u5B8C\u6210: ${count} \u6761`);
}
async function migratePatrolRecords() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB patrol_records \u2192 fire_patrol_record");
  const [oldRows] = await database_default.query("SELECT * FROM patrol_records");
  let count = 0;
  for (const row of oldRows) {
    try {
      ...
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const [result] = await database_default.query(
        `INSERT INTO fire_patrol_record
         (plan_id, patrol_no, unit_id, patrol_user_id, patrol_date, result,
          abnormal_desc, photos, signature, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          planNewId,
          row.patrol_no || `PR${Date.now()}_${count}_${Math.random().toString(36).slice(2, 6)}`,
          unitNewId,
          parseIntSafe(row.patrol_user_id) || null,
          toDate(row.patrol_date),
          mapStatus(row.result, { normal: 1, abnormal: 0, "1": 1, "0": 0 }),
          row.abnormal_desc || null,
          row.photos || null,
          row.signature || null
        ] }
      );
      ...
      logger_default.error(`[Migration] patrol_records \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] patrol_records \u2192 fire_patrol_record \u5B8C\u6210: ${count} \u6761`);
}
async function migrateHazards() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB hazards \u2192 fire_hazard");
  const [oldRows] = await database_default.query("SELECT * FROM hazards");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const [result] = await database_default.query(
        `INSERT INTO fire_hazard
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.hazard_no || `HZ${Date.now()}_${count}_${Math.random().toString(36).slice(2, 6)}`,
          unitNewId,
          parseIntSafe(row.hazard_type) || 1,
          row.description || null,
          parseIntSafe(row.level) || 1,
          row.photos || null,
          mapStatus(row.status, { pending: 0, processing: 1, done: 2, closed: 3, "0": 0, "1": 1, "2": 2, "3": 3 }),
          toDate(row.deadline),
          parseIntSafe(row.rectifier_id) || null,
          toDate(row.rectification_date),
          row.before_photo || null,
          row.after_photo || null
        ] }
      );
      const newId = result.insertId;
      ...
      logger_default.error(`[Migration] hazards \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] hazards \u2192 fire_hazard \u5B8C\u6210: ${count} \u6761`);
}
async function migratePlans() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB plans \u2192 fire_emergency_plan");
  const [oldRows] = await database_default.query("SELECT * FROM plans");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const [result] = await database_default.query(
        `INSERT INTO fire_emergency_plan
         (plan_name, unit_id, plan_type, content,
          status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.name || "\u672A\u547D\u540D",
          unitNewId,
          parseIntSafe(row.plan_type) || 1,
          row.content || null,
          mapStatus(row.status, { active: 1, inactive: 0, "1": 1, "0": 0 })
        ] }
      );
      ...
      logger_default.error(`[Migration] plans \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] plans \u2192 fire_emergency_plan \u5B8C\u6210: ${count} \u6761`);
}
async function migrateDrills() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB drills \u2192 fire_emergency_drill");
  const [oldRows] = await database_default.query("SELECT * FROM drills");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      ...
      const [result] = await database_default.query(
        `INSERT INTO fire_emergency_drill
         (plan_id, unit_id, drill_type, drill_date,
          location, participant_count, summary, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          planNewId,
          unitNewId,
          parseIntSafe(row.drill_type) || 1,
          toDate(row.drill_date),
          row.location || null,
          parseIntSafe(row.participant_count) || 0,
          row.summary || null,
          mapStatus(row.status, { active: 1, inactive: 0, "1": 1, "0": 0 })
        ] }
      );
      ...
      logger_default.error(`[Migration] drills \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] drills \u2192 fire_emergency_drill \u5B8C\u6210: ${count} \u6761`);
}
async function migrateInspections() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB inspections \u2192 fire_inspection");
  const [oldRows] = await database_default.query("SELECT * FROM inspections");
  let count = 0;
  for (const row of oldRows) {
    try {
      const unitNewId = row.unit_id ? await getNewId("fire_unit", String(row.unit_id)) : null;
      const [result] = await database_default.query(
        `INSERT INTO fire_inspection
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.inspect_no || `INSP${Date.now()}_${count}_${Math.random().toString(36).slice(2, 6)}`,
          unitNewId,
          parseIntSafe(row.inspect_type) || 1,
          row.inspector || null,
          toDate(row.inspect_date),
          row.items || null,
      ...
      logger_default.error(`[Migration] inspections \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] inspections \u2192 fire_inspection \u5B8C\u6210: ${count} \u6761`);
}
async function migrateDocuments() {
  logger_default.info("[Migration] \u5F00\u59CB\u8FC1\u79FB documents \u2192 fire_knowledge_doc");
  const [oldRows] = await database_default.query("SELECT * FROM documents");
  let count = 0;
  for (const row of oldRows) {
    try {
      const [result] = await database_default.query(
        `INSERT INTO fire_knowledge_doc
         (title, doc_type, category, content, file_url,
          status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        { replacements: [
          row.title || row.name || "\u672A\u547D\u540D",
          parseIntSafe(row.doc_type) || 1,
          row.category || null,
          row.content || null,
          row.file_url || null,
          mapStatus(row.status, { active: 1, inactive: 0, "1": 1, "0": 0 })
      ...
      logger_default.error(`[Migration] documents \u8FC1\u79FB\u5931\u8D25 id=${row.id}: ${err.message}`);
    }
  }
  logger_default.info(`[Migration] documents \u2192 fire_knowledge_doc \u5B8C\u6210: ${count} \u6761`);
}
async function runMigration() {
  logger_default.info("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
  logger_default.info("  \u6570\u636E\u8FC1\u79FB\u5F00\u59CB\uFF1A\u65E7\u7248\u8868 \u2192 \u65B0\u7248\u8868");
  logger_default.info("  \u26A0\uFE0F  \u751F\u4EA7\u73AF\u5883\u8FD0\u884C\u524D\u8BF7\u52A1\u5FC5\u5907\u4EFD\u6570\u636E\u5E93\uFF01");
  logger_default.info("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
  try {
    await database_default.authenticate();
    logger_default.info("[Migration] \u6570\u636E\u5E93\u8FDE\u63A5\u6210\u529F");
    await prepareLegacyTables();
    ...
    await migratePatrolPlans();
    await migratePatrolRecords();
    ...
    logger_default.info("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
    logger_default.info("  \u2705 \u6570\u636E\u8FC1\u79FB\u5168\u90E8\u5B8C\u6210");
    logger_default.info("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
  } catch (err) {
    logger_default.error(`[Migration] \u8FC1\u79FB\u5931\u8D25: ${err.message}`);
    throw err;
  } finally {
    await database_default.close();
  }
}
