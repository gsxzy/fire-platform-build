...
interface RouteMetrics {
  ...
  maxDuration: number;
  durations: number[]; // 最近 1000 个请求的延迟样本（用于 P99/P95）
...
class MetricsCollector {
  ...
  record(method: string, path: string, status: number, duration: number, slowThreshold = 1000) {
    ...
    if (duration > m.maxDuration) m.maxDuration = duration;
    if (duration > slowThreshold) m.slow++;
    if (status >= 400) {
      m.errors++;
      this.totalErrors++;
    }
