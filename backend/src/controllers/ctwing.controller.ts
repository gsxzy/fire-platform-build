...
/** 保存 CTWing 原始推送日志 */
async function saveRawLog(deviceId: string, msgType: string, rawBody: unknown) {
  try {
...
/** 解析设备标识 */
function resolveDeviceId(body: Record<string, unknown>): string {
  return String(
...
/** 解析 CTWing 数据 */
function parseCtwingBody(body: Record<string, unknown>) {
...
function parseThresholds(configStr: string | null | undefined): Record<string, number> {
  try {
...
/** 创建告警 */
async function createCtwingAlarm(parsed: ReturnType<typeof parseCtwingBody>, iotDevice: any) {
...
/** 保存 ISNB 解析后的遥测数据 */
async function saveIsnbTelemetry(iotDeviceId: number, frame: IsnbParsedFrame) {
  try {
    await sequelize.query(
        ...
        replacements: [
          ...
          (() => {
            ...
            if (ch02) {
              ...
              if (vk != null) return vk * 1000; // MPa → kPa
              if (ch02.paramValue != null) return (ch02.rawParamValue ?? 0) * 100; // 0.1MPa → kPa
            }
            return null;
          })(),
...
function resolveIsnbDeviceType(devType?: number): string {
  if (devType === 0x02 || devType === 0x82) return 'hikvision-smoke';      // 烟感 / 双通道烟感
  if (devType === 0x86 || devType === 0x88) return 'hikvision-pressure';   // 一体式用水 / 一体化用水（压力）
  if (devType === 0x03 || devType === 0x83) return 'hikvision-gas';        // 燃气 / 双通道燃气
  if (devType === 0x8c) return 'hikvision-co';                              // 一氧化碳
  if (devType === 0x89) return 'hikvision-manual';                          // 手报
  if (devType === 0x8a) return 'hikvision-sound-light';                     // 声光
