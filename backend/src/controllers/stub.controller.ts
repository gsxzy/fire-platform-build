...
import { sanitizePagination, MAX_PAGE_SIZE } from '@/utils/validator';
...
/* ───── 通用单条查询 ───── */
async function queryById(tableName: string, id: string) {
  assertTable(tableName);
  const [rows] = await sequelize.query(
...
/* ───── 通用创建 ───── */
async function createRow(req: Request, tableName: string) {
  assertTable(tableName);
  const validFields = await getTableColumns(tableName);
  if (!validFields) {
    throw new Error(`无法获取表 ${tableName} 的列信息，拒绝创建`);
  }
  const cols = Object.keys(req.body).filter(k => validFields.includes(k));
...
/* ───── 通用更新 ───── */
async function updateRow(req: Request, tableName: string, id: string) {
  assertTable(tableName);
  const validFields = await getTableColumns(tableName);
  if (!validFields) {
    throw new Error(`无法获取表 ${tableName} 的列信息，拒绝更新`);
  }
  const cols = Object.keys(req.body).filter(k => k !== 'id' && validFields.includes(k));
  const vals = cols.map(k => req.body[k]);
  if (!cols.length) return ok(null);
...
function makeList(table: string) {
...
function makeById(table: string) {
  return async (req: Request, res: Response) => {
...
function makeCreate(table: string) {
  return async (req: Request, res: Response) => {
