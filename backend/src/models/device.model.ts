...
export const Device = sequelize.define('device', {
  ...
  unit_id: { type: DataTypes.BIGINT, allowNull: true, comment: '未分配档案可为空' },
  ...
  status: { type: DataTypes.SMALLINT, defaultValue: 1, comment: '运行状态：1正常 2故障 3离线 4报废 0停用' },
  lifecycle_status: {
  ...
  building_id: { type: DataTypes.INTEGER, allowNull: true, comment: '所属建筑ID（分配阶段）' },
  floor_id: { type: DataTypes.INTEGER, allowNull: true, comment: '所属楼层ID（分配阶段）' },
  point_id: { type: DataTypes.INTEGER, allowNull: true, comment: '平面图点位ID（分配阶段）' },
  /* 新增字段：商用交付补齐 */
  remark: DataTypes.TEXT,
  config: DataTypes.TEXT,
  online_status: { type: DataTypes.SMALLINT, defaultValue: 0, comment: '在线状态：0离线 1在线' },
  ...
  indexes: [
    ...
    { fields: ['lifecycle_status', 'created_at'] },
    /* 复合索引：设备编号/名称/SN 联合搜索 */
    { fields: ['device_no', 'device_name', 'device_sn'] },
    /* 网关关联索引：查询传输装置关联的主机 */
    { fields: ['gateway_id'] },
  ],
});
