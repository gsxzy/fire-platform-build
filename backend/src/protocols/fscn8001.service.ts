...
/* ───── BCD 时间戳编解码 ───── */
export function decodeBcdTimestamp(buf: Buffer): string {
...
export function encodeBcdTimestamp(date: Date): Buffer {
...
/* ───── 校验和计算 ───── */
export function calcChecksum(buf: Buffer, start: number, end: number): number {
  let sum = 0;
...
/* ───── 构建确认帧（关键：防止 2 分钟掉线）──── */
export function buildAckFrame(reqFrame: ParsedFrame): Buffer {
  ...
  if (reqFrame.isFscnFormat) {
    ...
    let off = 0;
    resp.writeUInt16BE(0x4040, off); off += 2;
    resp.writeUInt16LE(reqFrame.seq, off); off += 2;
    reqFrame.version.copy(resp, off); off += 2;
    const nowTs = encodeBcdTimestamp(new Date());
    nowTs.copy(resp, off); off += 6;
    reqFrame.dstAddr.copy(resp, off); off += 6;
    reqFrame.srcAddr.copy(resp, off); off += 6;
    resp.writeUInt16LE(respDataLen, off); off += 2;
    resp.writeUInt8(respCmd, off); off += 1;
    respData.copy(resp, off); off += respDataLen;
    const cs = calcChecksum(resp, 2, off);
    resp.writeUInt8(cs, off); off += 1;
    resp.writeUInt16BE(0x2323, off); off += 2;
    logger.debug(`[FSCN8001] -> 确认帧[FSCN] cmd=0x${respCmd.toString(16).padStart(2,'0').toUpperCase()} seq=${reqFrame.seq}`);
    return resp;
  } else {
    const resp = Buffer.alloc(8 + respDataLen);
    let off = 0;
    resp.writeUInt16BE(0x4040, off); off += 2;
    resp.writeUInt8(reqFrame.raw[2], off); off += 1;
    resp.writeUInt8(reqFrame.raw[3], off); off += 1;
    resp.writeUInt8(respCmd, off); off += 1;
    respData.copy(resp, off); off += respDataLen;
    const cs = calcChecksum(resp, 2, off);
    resp.writeUInt8(cs, off); off += 1;
    resp.writeUInt16BE(0x2323, off); off += 2;
    logger.debug(`[FSCN8001] -> 确认帧[GB26875] cmd=0x${respCmd.toString(16).padStart(2,'0').toUpperCase()} seq=${reqFrame.seq}`);
    return resp;
  }
}
...
export class FrameParser {
  ...
  push(chunk: Buffer): ParsedFrame[] {
    ...
    if (this.buffer.length > this.MAX_BUFFER) {
      logger.warn(`[FSCN8001] 缓冲区超限 ${this.buffer.length} bytes，清空`);
  ...
  private extractFrame(): ParsedFrame | null {
    ...
    if (start > 0) {
      logger.warn(`[FSCN8001] 丢弃帧头前 ${start} 字节垃圾数据`);
  ...
  private parseFrame(buf: Buffer): ParsedFrame | null {
    const header = buf.readUInt16BE(0);
    const footer = buf.readUInt16BE(buf.length - 2);
    if (header !== 0x4040) {
      logger.error(`[FSCN8001] 帧头错误: 期望=0x4040, 实际=0x${header.toString(16).padStart(4, '0')}`);
      return null;
    }
    if (footer !== 0x2323) {
      logger.error(`[FSCN8001] 帧尾错误: 期望=0x2323, 实际=0x${footer.toString(16).padStart(4, '0')}`);
      return null;
    }

    const isFscnFormat = buf.length >= 30 && buf[4] === 0x01;

    let cmd: number, seq: number, version: Buffer, timestamp: Buffer;
    let srcAddr: Buffer, dstAddr: Buffer, deviceIdBuf: Buffer;
    let dataLen: number, data: Buffer;
    ...
    if (checksum !== expectedCs) {
      logger.error(`[FSCN8001] 校验和错误: 计算=0x${expectedCs.toString(16).padStart(2,'0').toUpperCase()}, 实际=0x${checksum.toString(16).padStart(2,'0').toUpperCase()}`);
      return null;
    }
...
export function parseDeviceStatus(data: Buffer): DeviceStatusEvent[] {
  ...
  if (typeFlag !== 0x02) {
    return [{ typeFlag, count, raw: data.subarray(2).toString('hex').toUpperCase() } as any];
  }

  const OBJ_LEN = 14; // GB26875 标准格式：系统类型(1)+系统地址(1)+部件类型(1)+部件地址(4)+部件状态(1)+时间(6)=14字节
  for (let i = 0; i < count; i++) {
    const off = 2 + i * OBJ_LEN;
    if (off + OBJ_LEN > data.length) break;
    ...
    let alarmLevel = 'low';
    if (isFire) { alarmType = 'fire'; alarmLevel = 'high'; }
    else if (isFault) { alarmType = 'fault'; alarmLevel = 'normal'; }
    else if (isSupervisory) { alarmType = 'supervisory'; alarmLevel = 'low'; }
    else if (isFeedback) { alarmType = 'feedback'; alarmLevel = 'low'; }
    else if (isShield) { alarmType = 'shield'; alarmLevel = 'low'; }
    else if (isPre) { alarmType = 'pre'; alarmLevel = 'normal'; }
    else if (isDelay) { alarmType = 'delay'; alarmLevel = 'low'; }

    events.push({
      sysType, sysAddr, devType, devTypeName: DEVICE_TYPE_NAMES[devType] || `未知类型(0x${devType.toString(16).padStart(2, '0').toUpperCase()})`,
      loopNo, pointNo,
...
/* ───── 协议事件发射器 ───── */
export const fscn8001Protocol = new EventEmitter();
