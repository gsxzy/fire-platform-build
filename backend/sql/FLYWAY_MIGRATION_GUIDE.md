...
> ⚠️ 服务器无法访问 `repo1.maven.org`，需**本地下载后上传**。

**步骤 A（本地机器执行）**：
```bash
# 在有网络的机器上下载 Flyway CLI
...
3. **MySQL 5.7 兼容**：目标环境若为 MySQL 5.7，禁止使用 `ADD COLUMN IF NOT EXISTS`、`ADD INDEX IF NOT EXISTS`、`ADD CONSTRAINT IF NOT EXISTS`。应使用 V050 风格的存储过程动态检测。
4. **无破坏性变更**：生产环境禁止 DROP TABLE / DROP COLUMN（除非双版本兼容）
5. **注释头**：每个文件必须包含 Flyway Migration 注释头
6. **测试验证**：新脚本必须先在测试环境执行通过后，再上生产

---

## 七、MySQL 5.7 兼容说明

部分迁移文件（V036~V045、V049、V052~V054）使用了 `IF NOT EXISTS` 语法（MySQL 8.0+），在 MySQL 5.7 上会报错。

**生产环境现状**：当前生产库已基线化到 V060，上述文件不会再执行。

**新环境部署**：若目标环境为 MySQL 5.7，请使用 `*_mysql57.sql` 替代版本：

| 原文件 | MySQL 5.7 兼容版 |
|--------|-----------------|
| `V049__optimize_device_indexes_20260515.sql` | `V049__optimize_device_indexes_20260515_mysql57.sql` |
| `V052__add_foreign_keys_and_indexes.sql` | `V052__add_foreign_keys_and_indexes_mysql57.sql` |
| `V053__optimize_production_indexes_and_events.sql` | `V053__optimize_production_indexes_and_events_mysql57.sql` |
| `V054__optimize_v2_indexes_and_analyze.sql` | `V054__optimize_v2_indexes_and_analyze_mysql57.sql` |

> 注：V036~V045 的兼容版本待补充，如需新环境部署请优先使用 V050 风格的存储过程模板改写。
...
2. **Git 历史中的密码**：若密码曾提交到 git，即使已删除文件，仍可通过 `git log --all --full-history -p -S '密码'` 找回。建议定期轮换密码。
3. **服务器文件权限**：敏感配置文件应设为 `600` 权限：
   ```bash
   chmod 600 /opt/my-fire-api-new/.env
   chmod 600 /opt/wvp/application.yml
   chmod 600 backend/sql/flyway.conf
   ```
