...
| 项目 | 值 |
|------|-----|
| 框架 | Express 4.19 + Sequelize 6.37 + PostgreSQL + TDengine + Redis + WebSocket |
| Node.js | `20.20.0` |
...
### 业务数据库（PostgreSQL + Redis + TDengine）
...
| 密码 | `<REDIS_PASSWORD>` |
| **TDengine** | |
| 端口 | `6041`（REST API）/ `6030`（原生） |
| 数据库 | `fire_platform_ts` |
| 用户 | `root` |
| 密码 | `taosdata` |
...
### 业务 PostgreSQL 查询
```bash
sudo -u postgres psql -c "\dt" fire_platform
# 或
PGPASSWORD=<DB_PASSWORD> psql -U fire_user -d fire_platform -c "SELECT tablename FROM pg_tables WHERE schemaname='public';"
```
...
```bash
# PostgreSQL 备份
pg_dump -U fire_user -d fire_platform > /backup/fire_platform_$(date +%Y%m%d_%H%M%S).sql

# TDengine 备份（如需要）
taosdump -D fire_platform_ts -o /backup/tdengine_$(date +%Y%m%d_%H%M%S)
```
...
### 生产环境执行
```bash
# 1. 数据清理与补全（PostgreSQL）
PGPASSWORD=<DB_PASSWORD> psql -U fire_user -d fire_platform -f backend/sql/archive/V058__cleanup_device_orphan_20260515.sql

# 2. 索引优化（若之前未执行）
PGPASSWORD=<DB_PASSWORD> psql -U fire_user -d fire_platform -f backend/sql/archive/V049__optimize_device_indexes_20260515.sql
```
...
```sql
-- CTWing 原始推送日志表（已有，由 Sequelize 模型自动创建）
-- CREATE TABLE IF NOT EXISTS ctwing_raw_log (...)

-- ISNB 遥测数据表（用于历史曲线，PostgreSQL 语法）
CREATE TABLE IF NOT EXISTS iot_telemetry (
  ...
  has_alarm SMALLINT DEFAULT 0,
  has_fault SMALLINT DEFAULT 0,
  raw_hex TEXT DEFAULT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_iot_device_id ON iot_telemetry(iot_device_id);
CREATE INDEX IF NOT EXISTS idx_created_at ON iot_telemetry(created_at);
```
...
### ✅ P1 — MySQL 迁移体系归档完成（2026-05-22）

**状态**：已完成
**背景**：V001~V064 全部基于 MySQL 语法，与新的 PostgreSQL 架构不兼容。

**处理方式**：
- 所有 MySQL 迁移脚本（V001~V064 及 4 个 Sequelize `.js`）已归档至 `backend/sql/archive/`
- 新环境使用 `backend/sql/baseline/V000__postgresql_baseline.sql` 进行初始化
- Flyway 从 V065 开始接管，全部使用 PostgreSQL 语法
- **无需再维护 MySQL 5.7 兼容版本**
...
|-----------|----------|----------|----------|
| PostgreSQL fire_user | `.env`、Flyway 配置 | 业务主库 | 修改后需同步后端、Flyway 配置、基线脚本 |
...
### 🟢 P2 — 日志自动清理（PostgreSQL 替代方案）

**状态**：待评估
**背景**：原 MySQL Event Scheduler 方案已废弃，PostgreSQL 需采用替代方案。

**可选方案**：
1. **PostgreSQL pg_cron 扩展**（需安装）
2. **Linux cron + psql 脚本**
3. **应用层定时任务**（Node.js node-cron 在后端执行清理）

**推荐**：方案 3（应用层），已在 `backend/src/cron/` 中实现，无需数据库层依赖。

**检查现有 cron 任务**：
```bash
psql -U fire_user -d fire_platform -c "SELECT * FROM cron.job;"  # 如安装了 pg_cron
# 或查看后端日志：
tail -f /opt/my-fire-api-new/logs/combined.log | grep -i "cleanup\|ttl"
```
...
- `/health`（公开）从简单字符串响应升级为**依赖健康探测**：
  - 检查 PostgreSQL 连接：`db.authenticate()`
...
或手动执行：
```bash
PGPASSWORD=<DB_PASSWORD> psql -U fire_user -d fire_platform -f backend/sql/archive/V063__enhance_duty_center.sql
```
...
## 十九、三数据库架构迁移完成（2026-05-21）

### 背景
将原单一 MySQL 8.0 架构升级为 **PostgreSQL 15（业务数据）+ TDengine 3.0.7.0（时序数据）+ Redis 7（缓存）** 的三库架构。
...
| 数据库 | `fire_platform` |
| 用户 | `fire_user` |
| 表数 | 70 张（Sequelize 模型）+ 9 张（原生 SQL） |
| 迁移行数 | ~2,200+（核心配置表） |

**关键变更**：
- Sequelize dialect: `mysql` → `postgres`
- 驱动: `mysql2` 移除，`pg` + `pg-hstore` 添加
- 数据类型映射: `BIGINT.UNSIGNED`→`BIGINT`, `TINYINT`→`SMALLINT`, `TEXT('long')`→`TEXT`, `JSON`→`JSONB`
- 自增: `AUTO_INCREMENT` → `GENERATED ALWAYS AS IDENTITY`
- 索引名全局唯一: 添加表前缀如 `idx_alarm_status`
- 移除模型级 `charset`/`collate`（PostgreSQL 使用数据库级编码）

### 2. TDengine 时序数据库

| 项目 | 值 |
|------|-----|
| 版本 | `3.0.7.0`（v3.3.6.6 在 OpenCloudOS 9.4 上安装失败） |
| 端口 | `6041`（REST API） |
| 数据库 | `fire_platform_ts` |
| 超级表 | `stb_telemetry`（遥测数据）、`stb_raw_log`（原始报文） |

**历史数据迁移结果**：

| 来源表 | MySQL 总行数 | TDengine 行数 | 状态 |
|--------|-------------|--------------|------|
| `fscn8001_raw_log` | 252,480 | **252,480** | ✅ 完整 |
| `gb26875_raw_log` | 120,598 | **120,598** | ✅ 完整 |
| `ctwing_raw_log` | 404 | **404** | ✅ 完整 |
| `iot_telemetry` | 92 | **87** | ✅ 基本完整（5 行缺失可接受） |
| **合计** | **373,574** | **373,569** | ✅ |

**关键技术问题与修复**：
1. **时间戳重复导致数据丢失**：fscn8001/gb26875 的大量历史记录时间戳完全相同（批量导入数据集中在同一秒内）。TDengine 同一子表相同时间戳视为重复，仅保留一条。
   - **修复**：迁移脚本为每条记录添加毫秒级递增偏移量（`base_ts + row_index ms`），确保时间戳唯一性。
2. **REST API 307 重定向导致 401**：`baseURL` 末尾带 `/` 时 axios 重定向丢失 Basic Auth 凭证。
   - **修复**：`baseURL: ${TD_URL}/rest/sql/${DB}`（无尾部斜杠）。
3. **编码问题**：Python `requests` 默认发送 `latin-1`，中文 JSON 会编码错误。
   - **修复**：SQL body 显式 `.encode('utf-8')`。
4. **子表名大小写**：TDengine 自动将表名转小写，脚本中使用 `.lower()` 保持一致。

### 3. MySQL 备份保留

- 原 `fire_platform` MySQL 数据库完整保留作为备份
- `wvp` 数据库（WVP-PRO 使用）仍在 MySQL 8.0 Docker 容器中运行，端口 `3307`

### 4. 服务状态

```bash
# 检查三库状态
systemctl status postgresql      # active (OpenCloudOS 服务名)
systemctl status taosd           # active, enabled
systemctl status taosadapter     # active, enabled
redis-cli ping                   # PONG
pm2 status fire-platform         # online
```
...
## 二十、迁移体系基线化重构（2026-05-22）

### 背景
运行时已于 2026-05-21 完成 MySQL → PostgreSQL + Redis + TDengine 三库切换，但 schema 初始化与演进体系（Flyway V001~V064 + Sequelize .js + Docker Compose）仍完全停留在 MySQL 时代。新环境无法部署，Docker 一键启动失效。

### 处理方案：基线化 + 归档历史 + 增量迁移

| 改造项 | 操作 | 状态 |
|--------|------|------|
| Flyway V001~V064 | 全部归档至 `backend/sql/archive/` | ✅ |
| Sequelize .js 迁移（4个） | 全部归档至 `backend/sql/archive/` | ✅ |
| PostgreSQL 基线脚本 | 新建 `backend/sql/baseline/V000__postgresql_baseline.sql` | ✅ |
| docker-compose.yml | MySQL 8.0 → PostgreSQL 15 + TDengine 3.3 | ✅ |
| docker/mysql/ | 删除 MySQL Docker 配置目录 | ✅ |
| device.model.ts | 移除 `INTEGER.UNSIGNED`（3处） | ✅ |
| knowledge.model.ts | 移除 `FULLTEXT` 索引类型声明 | ✅ |
| 代码注释 | `gb28181.controller.ts` / `floorPlanApp.routes.ts` MySQL 引用更新 | ✅ |
| backend/sql/README.md | 重写迁移文档，规范 V065+ PostgreSQL 语法 | ✅ |

### 基线脚本内容

`backend/sql/baseline/V000__postgresql_baseline.sql` 包含：
- **扩展安装**：`pg_trgm`（用于相似度搜索）
- **原生 SQL 管理表**（8张）：
  - `gb26875_device` / `gb26875_alarm`
  - `fscn8001_device` / `fscn8001_alarm`
  - `fire_device_heartbeat`
  - `fire_camera`
  - `sys_todo` / `sys_notice`
  - `sys_refresh_tokens`
- **高频业务索引**：fire_device、fire_alarm、fire_iot_device 等复合索引
- **PostgreSQL 特有索引**：
  - `idx_knowledge_fts`（GIN + `to_tsvector('simple', ...)`）全文检索
  - `uk_floor_camera`（唯一索引，支持 UPSERT）
...
# PostgreSQL 首次启动自动执行 initdb.d 中的基线脚本
...
| 环境 | 影响 | 操作 |
|------|------|------|
| **现有生产** | 零影响 | schema 已存在，继续从 V065 执行增量迁移 |
| **现有测试** | 零影响 | 同上 |
| **全新环境** | 正常 | 基线脚本 + Flyway baseline 后启动 |
...
# 新增
- postgres:15-alpine 服务（端口 5432，initdb.d 自动执行基线）
- tdengine:3.3.6.6 服务（端口 6030/6041）
- postgres_data / tdengine_data 卷
```
...
| 文件 | 修改 |
|------|------|
| `device.model.ts` | `INTEGER.UNSIGNED` → `INTEGER`（PostgreSQL 无 UNSIGNED） |
| `knowledge.model.ts` | 移除 `{ type: 'FULLTEXT', ... }`，改用基线脚本的 GIN 索引 |

### 增量迁移规范（V065+）

1. 强制 PostgreSQL 语法，禁止 `ENGINE=InnoDB` / `DEFAULT CHARSET`
2. 索引：`CREATE INDEX IF NOT EXISTS`
3. 列变更：`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`
4. 时间戳：`DEFAULT CURRENT_TIMESTAMP`，应用层维护 `updated_at`
5. JSON：使用 `JSONB` + `GIN` 索引
6. 全文检索：`USING GIN (to_tsvector('simple', ...))`
7. 自增主键：`GENERATED ALWAYS AS IDENTITY` 或 `SERIAL`
