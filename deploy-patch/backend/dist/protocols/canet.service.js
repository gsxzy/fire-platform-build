...
exports.FrameParser = exports.CanfdnetParser = void 0;
...
/* ───── CANFDNET 网络包流式解析器 ───── */
class CanfdnetParser {
    buffer = Buffer.alloc(0);
    MAX_BUFFER = 65536;
    push(chunk) {
        this.buffer = Buffer.concat([this.buffer, chunk]);
        if (this.buffer.length > this.MAX_BUFFER) {
            logger_1.default.warn(`[CANFDNET-Parser] 缓冲区超限 ${this.buffer.length} bytes，清空`);
        ...
        while (this.buffer.length >= 7) { // 最小包 = 头(1)+类型(1)+参数(1)+保留(1)+长度(2)+校验(1) = 7
            ...
            if (syncIdx > 0) {
                logger_1.default.warn(`[CANFDNET-Parser] 丢弃错位 ${syncIdx} 字节，重新同步`);
    ...
    parsePacket(buf) {
        ...
        if (checksum !== actualChecksum) {
            logger_1.default.warn(`[CANFDNET-Parser] BCC 校验失败: calc=0x${checksum.toString(16).padStart(2, '0')} actual=0x${actualChecksum.toString(16).padStart(2, '0')} type=0x${type.toString(16)}`);
            return null;
        }
    ...
    extractFrames(packet) {
        ...
        if (packet.type === 0x00) {
            ...
            // 总线利用率指示包，忽略
            logger_1.default.debug(`[CANFDNET-Parser] 总线利用率包，忽略`);
        }
        else {
            logger_1.default.debug(`[CANFDNET-Parser] 未知包类型: 0x${packet.type.toString(16)}`);
        }
        return frames;
    }
    parseCanMessage(buf) {
    ...
    canMessageToParsedFrame(msg) {
        // 跳过错误帧
        if (msg.info & CANFDNET_INFO_ERR) {
            logger_1.default.debug(`[CANFDNET-Parser] 跳过错误帧: id=0x${msg.id.toString(16)}`);
            return null;
        }
        // 跳过发送回显（避免重复处理）
    ...
    canfdMessageToParsedFrame(msg) {
        if (msg.info & CANFDNET_INFO_ERR) {
            ...
            if (now - this.lastErrorLog > 10000) {
                ...
                logger_1.default.info(`[CANFDNET-Parser] 错误帧: 总线=${statusMap[busStatus] || '0x' + busStatus.toString(16)} ` +
    ...
    buildParsedFrame(id, info, dataLen, data) {
...
exports.CanfdnetParser = CanfdnetParser;
/* ───── 旧版 FrameParser（保留兼容）──── */
class FrameParser {
    buffer = Buffer.alloc(0);
    MAX_BUFFER = 65536;
