...
class CanetServer {
    ...
    handleConnection(socket) {
        ...
        const parser = new canet_service_1.CanfdnetParser();
        const conn = {
            socket,
            clientIp,
            clientPort,
        ...
        socket.on('data', (chunk) => {
            ...
            logger_1.default.info(`[CANET] data事件 conn=${connId} bytes=${chunk.length}`);
            const frames = parser.push(chunk);
            if (frames.length > 0) {
                logger_1.default.info(`[CANET] parser返回帧数 conn=${connId} count=${frames.length}`);
    ...
    /* ───── 赋安 FS5102 CAN 报警帧解析 ───── */
    parseFuanAlarmFrame(frame) {
        // 赋安 CAN 报警帧特征：
        // - 扩展帧，数据长度=8
        // - 数据格式：D0=回路, D1=点位, D2=状态, D3=设备类型, D4-D7=保留
        // - 状态：0x01=火警, 0x02=故障
        if (!frame.isExtended || frame.dataLen !== 8)
            return null;
        const d = frame.data;
        const loop = d[0];
        const point = d[1];
    ...
    async handleFrame(frame, conn) {
        try {
            logger_1.default.info(`[CANET] 收到CAN帧 conn=${conn.connId} raw=${frame.hex} canId=0x${frame.canIdHex} data=${frame.dataHex} ext=${frame.isExtended} rtr=${frame.isRtr} len=${frame.dataLen}`);
            // 扩展ID详细解析（用于调试协议）
            if (frame.isExtended) {
                ...
                const rsv = id & 0xFFF;
                logger_1.default.info(`[CANET] 扩展ID解析: src=0x${src.toString(16).padStart(2, '0')} tgt=0x${tgt.toString(16).padStart(2, '0')} func=0x${func.toString(16)} rsv=0x${rsv.toString(16).padStart(3, '0')} id=0x${id.toString(16).padStart(8, '0')}`);
            }
            // 记录所有帧（包括标准帧，用于协议分析）
            if (frame.isRtr) {
                logger_1.default.info(`[CANET] 跳过远程帧: ext=${frame.isExtended} len=${frame.dataLen}`);
                return;
            }
            // 解析赋安 FECbus 广播帧
            const fecbus = (0, canet_service_1.parseFecbusFrame)(frame);
            if (fecbus) {
                logger_1.default.info(`[CANET] FECbus解析成功: src=0x${fecbus.sourceAddr.toString(16).padStart(2, '0')} tgt=0x${fecbus.targetAddr.toString(16).padStart(2, '0')} func=0x${fecbus.functionCode.toString(16)} loop=${fecbus.loop} point=${fecbus.point} status=0x${fecbus.status.toString(16)} devType=0x${fecbus.deviceType.toString(16)}`);
                // 只处理功能码=0x06（设备状态广播）且目标地址=0x00（广播）
                if (fecbus.functionCode !== 0x06 || fecbus.targetAddr !== 0x00) {
                    logger_1.default.info(`[CANET] 跳过非目标功能码/地址: func=0x${fecbus.functionCode.toString(16)} tgt=0x${fecbus.targetAddr.toString(16)}`);
                    return;
                }
            ...
            if (fuanAlarm) {
                logger_1.default.info(`[CANET] 赋安CAN报警帧解析成功: 回路${fuanAlarm.loop} 点位${fuanAlarm.point} 状态=0x${fuanAlarm.status.toString(16)} 类型=0x${fuanAlarm.deviceType.toString(16)}`);
                const binding = await this.findDeviceBinding(conn.clientIp, conn.clientPort);
            ...
            logger_1.default.info(`[CANET] 非 FECbus 广播帧: ID=0x${frame.canIdHex} data=${frame.dataHex}`);
        }
        catch (err) {
        ...
        try {
         ...
         LIMIT 1`, { replacements: [clientIp, `%CANET%`], type: sequelize_1.QueryTypes.SELECT });
            if (rows && rows.length > 0) {
                const r = rows[0];
                logger_1.default.info(`[CANET] 绑定成功(策略1/canet): device=${r.device_name} unit=${r.unit_name}`);
                return {
            ...
            const rows2 = await database_1.default.query(`SELECT
           i.id, i.device_sn, i.archive_device_id,
           d.id AS device_id, d.device_name, d.unit_id, u.unit_name
         FROM fire_iot_device i
         JOIN fire_device d ON i.archive_device_id = d.id
         ...
         WHERE i.ip_address = ?
         ORDER BY i.updated_at DESC
         LIMIT 1`, { replacements: [clientIp], type: sequelize_1.QueryTypes.SELECT });
            if (rows2 && rows2.length > 0) {
                const r = rows2[0];
                logger_1.default.info(`[CANET] 绑定成功(策略2/IP回退): device=${r.device_name} unit=${r.unit_name}`);
                return {
                    deviceId: r.device_id ?? null,
                    hostName: r.device_name ?? null,
                    unitId: r.unit_id ?? null,
            ...
            logger_1.default.warn(`[CANET] 未找到设备绑定: ip=${clientIp}`);
    ...
    /* ───── 创建统一告警 ───── */
    async createAlarm(type, level, binding, deviceName, location, deviceTypeName, connId, loop, point, rawHex) {
        const alarmNo = (0, alarmNo_1.generateAlarmNo)();
        const typeMap = { fire: 1, fault: 2 };
        const levelMap = { urgent: 3, high: 2, normal: 1 };
        logger_1.default.info(`[CANET] 正在创建告警: type=${type} level=${level} deviceName=${deviceName} unitId=${binding.unitId}`);
        try {
            ...
            logger_1.default.warn(`[CANET] 🚨 ${type === 'fire' ? '火警' : '故障'} 回路${loop} 点位${point} ${deviceTypeName} | conn=${connId}`);
        }
        catch (err) {
            logger_1.default.error(`[CANET] 告警创建失败: ${err.message}`);
        }
    }
    /* ───── 状态接口 ───── */
    getStats() {
