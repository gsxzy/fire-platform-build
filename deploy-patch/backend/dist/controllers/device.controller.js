...
exports.DeviceController = {
    async list(req, res) {
        ...
        if (needIot === 'true' || needIot === '1') {
            where[sequelize_1.Op.and] = [
                where[sequelize_1.Op.and],
                sequelize_1.Sequelize.literal(`EXISTS (SELECT 1 FROM fire_iot_device iot WHERE iot.archive_device_id = device.id AND iot.archive_device_id IS NOT NULL LIMIT 1)`)
            ];
        }
    ...
    async saveConfig(req, res) {
        ...
        const updateData = { config: configJson };
        // 若请求中包含 protocol_type，同步更新 fire_device.protocol_type
        if (body.protocol_type !== undefined) {
            updateData.protocol_type = body.protocol_type;
        }
        await models_1.Device.update(updateData, { where: { id: req.params.id } });
