...
async function seed() {
    try {
        ...
        /* ── 1. 组织架构 ── */
        await models_1.Department.bulkCreate([
        ...
        const perms = permissions_1.PERMISSION_CODES.map((code) => ({
            perm_name: permissions_1.PERMISSION_NAMES[code],
            perm_code: code,
            type: permissions_1.PERMISSION_TYPES[code],
        }));
        await models_1.Permission.bulkCreate(perms, { ignoreDuplicates: true });
        ...
        const opsRole = await models_1.Role.findOrCreate({
        ...
        // 分配所有权限给admin
        const allPerms = await models_1.Permission.findAll();
        await adminRole[0].setPermissions(allPerms.map((p) => p.id));
        // 运维人员角色：保留基础操作权限
        const opsPermCodes = [
        ...
        await opsRole[0].setPermissions(allPerms.filter((p) => opsPermCodes.includes(p.perm_code)).map((p) => p.id));
        ...
        if (!adminPassword) {
            ...
            await adminUser.setRoles([adminRole[0].id]);
            console.log('[Seed] Admin user created');
        }
        /* ── 5. 演示单位 ── */
        await models_1.Unit.bulkCreate([
        ...
        await models_1.Device.bulkCreate(devices, { ignoreDuplicates: true });
        ...
        await models_1.Alarm.bulkCreate(alarms, { ignoreDuplicates: true });
        console.log('[Seed] Alarms created');
        /* ── 8. 消控室 ── */
        await models_1.ControlRoom.bulkCreate([
        ...
        /* ── 9. 维保单位 ── */
        await models_1.MaintenanceCompany.bulkCreate([
        ...
        /* ── 10. 知识库 ── */
        await models_1.KnowledgeDoc.bulkCreate([
        ...
        /* ── 11. 系统配置 ── */
        await models_1.SystemConfig.bulkCreate([
        ...
        /* ── 12. 大屏配置 ── */
        await models_1.ScreenConfig.bulkCreate([
