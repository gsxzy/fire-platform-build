...
function extractIsnbHexFromCtwing(body) {
    ...
    for (const cand of topCandidates) {
        ...
        if (cand && typeof cand === 'object') {
            ...
            const svcData = cand.serviceData
                ?? cand.service_data;
            if (svcData && typeof svcData === 'object') {
                const svcRaw = svcData.rawData
                    ?? svcData.raw_data
                    ?? svcData.rawHex
                    ?? svcData.raw_hex;
            ...
            const services = cand.services;
            if (Array.isArray(services)) {
                for (const svc of services) {
                    if (svc && typeof svc === 'object') {
                        const svcDataObj = svc.data
                            ?? svc.serviceData;
                        if (svcDataObj && typeof svcDataObj === 'object') {
                            const svcRaw = svcDataObj.rawData
                                ?? svcDataObj.raw_data
                                ?? svcDataObj.rawHex
                                ?? svcDataObj.raw_hex;
                            if (typeof svcRaw === 'string' && isIsnbHexFrame(svcRaw)) {
