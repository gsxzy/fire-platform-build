...
exports.IoTProtocolService = void 0;
/**
 * ═══════════════════════════════════════════════════════════════════
 * IoT协议解析服务
...
class IoTProtocolService {
    ...
    static async parseMQTTMessage(topic, payload) {
        try {
            const parts = topic.split('/');
            const deviceSn = parts[parts.length - 1];
            const data = JSON.parse(payload.toString());
            const device = await models_1.IoTDevice.findOne({ where: { device_sn: deviceSn } });
            if (!device)
                return { success: false, msg: '设备未注册' };
            // 解析4G设备数据格式
            const parsed = this.parse4GDeviceData(data, device);
            // 更新设备状态
            await models_1.IoTDevice.update({ status: 1, last_online: new Date() }, { where: { id: device.id } });
            await models_1.Device.update({ last_online: new Date(), status: parsed.alarm ? 2 : 1 }, { where: { iot_id: deviceSn } });
            // 生成告警
            if (parsed.alarm) {
            ...
            logger_1.default.error(`[IoTProtocol] MQTT parse error: ${err.message}`);
            return { success: false, msg: err.message };
        }
    }
    // 4G设备数据解析（烟感、温感、水压、液位、防火门、电气火灾等）
    static parse4GDeviceData(raw, device) {
        const result = { raw, timestamp: new Date().toISOString(), alarm: false, values: {} };
        // 烟感探测器数据格式
        ...
        if (device.device_type?.includes('温感') || raw.temperature !== undefined) {
            ...
            if (raw.temperature > 60 || raw.alarm === 1) {
                result.alarm = true;
                result.alarmType = 1;
                result.alarmDesc = `温度异常 ${raw.temperature}°C`;
            }
        }
        // 水压传感器数据格式
        if (device.device_type?.includes('水压') || raw.pressure !== undefined) {
            ...
            if (raw.pressure < 0.1) {
                result.alarm = true;
                result.alarmType = 3;
                result.alarmDesc = `水压过低 ${raw.pressure}MPa`;
            }
            else if (raw.pressure > 1.2) {
                result.alarm = true;
                result.alarmType = 3;
                result.alarmDesc = `水压过高 ${raw.pressure}MPa`;
            }
        }
        // 液位传感器数据格式
        ...
        if (device.device_type?.includes('电气') || raw.leakageCurrent !== undefined) {
            ...
            if (raw.leakageCurrent > 300) {
                result.alarm = true;
                result.alarmType = 1;
                result.alarmDesc = `剩余电流超限 ${raw.leakageCurrent}mA`;
            }
            if (raw.cableTemp > 70) {
                result.alarm = true;
                result.alarmType = 3;
                result.alarmDesc = `线缆温度异常 ${raw.cableTemp}°C`;
            }
        }
        return result;
    }
    // 创建设备告警
    static async createDeviceAlarm(device, parsed) {
        const unitDevice = await models_1.Device.findOne({ where: { iot_id: device.device_sn } });
        await models_1.Alarm.create({
            alarm_no: (0, alarmNo_1.generateAlarmNo)(),
    ...
    static async readModbusDevice(ip, port, slaveId, registerAddr, quantity = 1) {
        return iot_1.iotGateway.readModbus(ip, port, registerAddr, quantity);
    }
    // SNMP协议读取（网关设备）
    static async readSNMPDevice(ip, community, oid) {
        return iot_1.iotGateway.readSNMP(ip, community, oid);
    }
    // 下发控制指令
    static async sendControlCommand(deviceSn, command) {
        return iot_1.iotGateway.sendCommand(deviceSn, command);
    }
    // 批量读取设备数据
    static async batchReadDevices(deviceIds) {
        const results = [];
        for (const id of deviceIds) {
            const device = await models_1.IoTDevice.findByPk(id);
            if (!device)
                continue;
            if (device.protocol_type === 'Modbus') {
                ...
                const config = JSON.parse(device.protocol_config || '{}');
                const r = await this.readSNMPDevice(config.ip, config.community, config.oid);
...
exports.IoTProtocolService = IoTProtocolService;
//# sourceMappingURL=iotProtocol.service.js.map