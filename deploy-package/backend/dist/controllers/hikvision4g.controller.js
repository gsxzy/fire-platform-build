...
async function createAlarm(parsed, iotDevice) {
    try {
        ...
        try {
            ...
            websocket_service_1.WebSocketService.broadcastSimple('new_alarm', pushPayload);
        }
        catch { /* ignore */ }
...
async function syncUnifiedDevice(deviceSn, ip, deviceType, state) {
    ...
    try {
        ...
        if (!lockRes?.acquired) {
            logger_1.default.warn(`[Hik4G] 获取锁失败: ${lockName}`);
            return null;
        }
        // 若档案已存在（按 device_sn 匹配），只更新状态
        const [rows] = await database_1.default.query(`SELECT id, device_no FROM fire_device WHERE device_sn = ? LIMIT 1`, { replacements: [deviceSn], type: 'SELECT' });
        const existing = rows?.[0];
        if (existing?.id) {
            await database_1.default.query(`UPDATE fire_device SET status = ?, lifecycle_status = 2, last_online = NOW(), protocol_type = 'Hikvision4G', updated_at = NOW() WHERE id = ?`, { replacements: [status, existing.id] });
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
           ...
           VALUES (?, ?, ?, ?, NULL, ?, 2, NOW(), 'Hikvision4G', NOW(), NOW())`, { replacements: [deviceNo, deviceSn, `${typeName}-${deviceSn.slice(-6)}`, typeName, status] });
                // 获取刚插入的 id
                const [newRows] = await database_1.default.query(`SELECT id FROM fire_device WHERE device_sn = ? ORDER BY id DESC LIMIT 1`, { replacements: [deviceSn], type: 'SELECT' });
