    ...
    async list(req, res) {
        ...
        if (needIot === 'true' || needIot === '1') {
            const iotRows = await models_1.IoTDevice.findAll({
                ...
                where: { archive_device_id: { [sequelize_1.Op.ne]: null } },
                raw: true,
            });
            const validIds = iotRows.map((r) => r.archive_device_id).filter(Boolean);
            if (validIds.length === 0) {
                (0, respond_1.sendPage)(res, req, [], 0, +pageNum, +pageSize);
                return;
            }
